...
class Main {
    ...
    public String errores(){
      String resp = "";
      Collections.sort(compile_error);
      for(Custom_Error error:compile_error)
          resp += error.msg;
      return resp;
    }
 ...
 public void compilar(String[] args) throws java.io.IOException {
    ...
    compile_error = new ArrayList<Custom_Error>();  
    String mis = recovery_string(archivo);
    IMips = new CodigoIntermedioTools();
    ...
    if (!parser_obj.has_main)
			  this.compile_error.add(new Custom_Error(0,"No tiene main!!"));    
		} catch (Exception e) {
      e.printStackTrace();
		}
    
    prueba flexer = new prueba(new FileReader(archivo));
    parser parser_obj2 = new parser(flexer);
    ...
    for(Custom_Error err: flexer.errores_flex)
        this.compile_error.add(new Custom_Error(err.linea,err.msg)); 
  }
